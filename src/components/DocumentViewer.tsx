...
export const DocumentViewer: React.FC = () => {
  ...
  const generatePDF = async (elementId: string): Promise<jsPDF | null> => {
    ...
    try {
      ...
      const baseScale = 2; // Always use high definition scale
      ...
      // Always return maximum high-definition PDF
  ...
  const isCompressedDocType = (docId: string) => {
    return false; // Always download in HD, no compression padding limit
  };
  ...
  return (
    <div className="flex flex-col h-full print:h-auto print:block relative bg-[#f8fafc]">
      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-gray-200/60 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 print:hidden sticky top-0 z-10 shadow-sm">
        <div className="flex items-center w-full sm:w-auto overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      <div className="flex-1 overflow-y-auto print:overflow-visible p-6 md:p-10 flex justify-center items-start print:p-0 print:block">
        <div id="preview-container" className="w-full max-w-[794px] h-fit print:max-w-full print:min-h-0 bg-white shadow-2xl shadow-gray-300/50 ring-1 ring-gray-900/5 print:shadow-none print:ring-0 relative rounded-sm select-none">
      ...
      <div className="fixed left-[-9999px] top-0 pointer-events-none opacity-0 print:hidden w-[794px] select-none">
        <div id="hidden-doc-AnnexureI" className="w-[794px] bg-white"><AnnexureI /></div>
        <div id="hidden-doc-ModelAgreement" className="w-[794px] bg-white"><ModelAgreement /></div>
        <div id="hidden-doc-Annexure3" className="w-[794px] bg-white"><Annexure3 /></div>
        <div id="hidden-doc-Receipt" className="w-[794px] bg-white"><Receipt /></div>
        <div id="hidden-doc-WCR" className="w-[794px] bg-white"><WCR /></div>
        <div id="hidden-doc-Invoice" className="w-[794px] bg-white"><Invoice /></div>
        <div id="hidden-doc-Proposal" className="w-[794px] bg-white"><Proposal /></div>
      </div>
    </div>
  );
};
