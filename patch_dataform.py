...
customer_details_target = """                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 gap-y-10">
                  {renderField('Name', 'name', 'text', '', 0)}
                  {renderField('Consumer number', 'consumerNumber', 'text', '', 1)}
                  {renderField('Email ID', 'emailId', 'email', '', 2)}
                  {renderField('Date', 'installationDate', 'date', '', 3)}
                  <div className="md:col-span-2">
                     {renderField('Installation Address', 'address', 'text', '', 4)}
                  </div>
                </div>"""

customer_details_replacement = """                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 gap-y-10">
                  {renderField('Name', 'name', 'text', '', 0)}
...
content = content.replace(customer_details_target, customer_details_replacement)

system_setup_target = """                 <div className="bg-gradient-to-b from-gray-50/50 to-white p-8 rounded-[2rem] border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-shadow duration-300">
...
system_setup_replacement = """                 <div className="bg-gradient-to-b from-gray-50/50 to-white p-8 rounded-[2rem] border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-shadow duration-300">
                 <h4 className="text-xs tracking-[0.25em] font-black text-gray-400 mb-8 flex items-center gap-3">
                    <span className="w-6 h-6 rounded-lg bg-emerald-100 flex items-center justify-center border border-emerald-200"><div className="w-2 h-2 bg-emerald-500 rounded-full shadow-[0_0_8px_#10b981]"></div></span> 
                    SOLAR PV DETAILS
                 </h4>
...
content = content.replace(system_setup_target, system_setup_replacement)
